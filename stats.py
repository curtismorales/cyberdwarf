...
def cohort_player_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    player_stats.columns = [f"{s}_mean" for s in SCORED_STATS] + [
...
def cross_join(
    df1: pd.DataFrame, df2: pd.DataFrame, suffixes: Optional[List[str]] = None
) -> pd.DataFrame:
    df1["tmp"] = 1
    df2["tmp"] = 1
    kwargs = {"on": "tmp"}
...
def score_cohort(df: pd.DataFrame) -> pd.DataFrame:
    ...
    combined = cross_join(player_stats, overall_stats, suffixes=["", "_overall"],)
    combined.index = combined["player"]

    # Calculate z-scores
    for s in SCORED_STATS:
        combined[f"{s}_z"] = (
            combined[f"{s}_mean"] - combined[f"{s}_mean_overall"]
        ) / combined[f"{s}_std_overall"]
